...
function GroupBadge({ group }: { group: unknown }) {
  ...
  return (
    <span style={{ fontSize: '12px', fontWeight: 700, color: inkOf(color), backgroundColor: color + '18', border: `1px solid ${color}40`, borderRadius: '8px', padding: '1px 6px', whiteSpace: 'nowrap' }}>
      {String(group)}
    </span>
  )
}
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ backgroundColor: '#F3EEFF', fontSize: '12.5px', fontWeight: 500, padding: '11px 14px', textAlign: 'left' as const, borderBottom: '1px solid #E8E4F1', whiteSpace: 'nowrap' as const, cursor: 'pointer', userSelect: 'none' as const, color: active ? '#0F766E' : '#655F7D' }}>
    ...
    <div>
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '28px' }}>
          {[
            { label: 'Toplam Küratör', value: curators.length, color: '#0F766E' },
            { label: 'Toplam Takipçi', value: fmt(totalFollowers), color: '#047857' },
            { label: 'Ort. Tavsiye Oranı', value: avgRate + '%', color: '#C2410C' },
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #E8E4F1', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#0F766E', boxShadow: 'none' }} />
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '15px', letterSpacing: '-0.01em', fontWeight: 600, color: '#17122B' }}>Küratör Listesi</span>
            <span style={{ marginLeft: 'auto', backgroundColor: '#F0FDFA', color: '#0F766E', border: '1px solid #D8D2E6', borderRadius: '9999px', padding: '2px 10px', fontSize: '14px', fontWeight: 600 }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', padding: '12px 20px', borderBottom: '1px solid #E8E4F1', alignItems: 'center' }}>
            ...
            {hasFilters && (
              <button onClick={() => { setSearch(''); setGroupFilter(''); setEmailFilter('') }} style={{ fontSize: '12px', color: '#655F7D', background: 'none', border: '1px solid #E8E4F1', borderRadius: '8px', padding: '7px 12px', cursor: 'pointer' }}>
                Temizle
              </button>
            )}
          </div>
                  ...
                  return (
                    <tr key={String(row.id ?? i)} style={{ borderBottom: i < sorted.length - 1 ? '1px solid #E8E4F1' : 'none', backgroundColor: i % 2 === 1 ? '#FAF9FD' : 'transparent' }}>
                      <td style={{ ...TD, paddingRight: '8px' }}>
                        <div style={{ width: '32px', height: '32px', borderRadius: '8px', backgroundColor: color + '22', border: `1px solid ${color}44`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, color: inkOf(color), flexShrink: 0 }}>
                          {initials(row.name)}
                        </div>
                      </td>
                      <td style={TD}>
                        <div style={{ fontWeight: 600, fontSize: '14px', color: '#17122B', maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{String(row.name ?? '—')}</div>
                        <div style={{ marginTop: '4px' }}><GroupBadge group={row.group_size} /></div>
                      </td>
                      <td style={TD}>
                        <span style={{ color: '#047857', fontWeight: 600, fontSize: '14px' }}>{fmt(row.followers)}</span>
                      </td>
                      <td style={TD}>
                        <span style={{ color: '#4A4462', fontWeight: 600, fontSize: '14px' }}>{fmt(row.total_reviews)}</span>
                      </td>
                      <td style={TD}>
                        {row.recommendation_rate
                          ? <span style={{ color: '#047857', fontWeight: 600, fontSize: '14px' }}>{String(row.recommendation_rate)}</span>
                          : <span style={{ color: '#655F7D' }}>—</span>}
                      </td>
                      <td style={TD}><ScoreBadge score={row.score} /></td>
                      <td style={TD}>
                        {isSteamCC ? (
                          <span style={{ fontSize: '12px', fontWeight: 700, color: '#1D4ED8', backgroundColor: '#EFF6FF', border: '1px solid #E8E4F1', borderRadius: '8px', padding: '2px 8px', whiteSpace: 'nowrap' }}>Steam CC</span>
                        ) : row.email ? (
                          <span style={{ color: '#6D28D9', fontFamily: 'monospace', fontSize: '12px', maxWidth: '160px', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{String(row.email)}</span>
                        ) : (
                          <span style={{ color: '#655F7D' }}>—</span>
                        )}
                      </td>
                      <td style={TD}>
                        {!!row.steam_link ? (
                          <a href={String(row.steam_link)} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 600, color: '#0F766E', backgroundColor: '#F0FDFA', border: '1px solid #E8E4F1', borderRadius: '8px', padding: '4px 8px', textDecoration: 'none', whiteSpace: 'nowrap' }}>
