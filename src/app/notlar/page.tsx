...
function truncate(v: unknown, len = 80) {
  ...
  return (
    <span style={{ fontSize: '13px', color: 'var(--text-2)' }}>
      {s.length > len ? s.slice(0, len) + '…' : s || <span style={{ color: 'var(--muted-foreground)' }}>—</span>}
    </span>
  )
}
...
export default async function NotlarPage() {
  ...
  const generalCols = [
    ...
    { key: 'category', label: 'Kategori', render: (v: unknown) => v ? <Badge variant="orange">{String(v)}</Badge> : <span style={{ color: 'var(--muted-foreground)' }}>—</span> },
    ...
    { key: 'author', label: 'Yazar', render: (v: unknown) => v ? <Badge variant="purple">{String(v)}</Badge> : <span style={{ color: 'var(--muted-foreground)' }}>—</span> },
      ...
      <div style={{ padding: '24px 32px' }}>
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--foreground)' }}>Genel Notlar</span>
            <span style={{ marginLeft: 'auto', backgroundColor: '#f59e0b20', color: 'var(--orange)', border: '1px solid #f59e0b40', borderRadius: '9999px', padding: '2px 10px', fontSize: '13px', fontWeight: 600 }}>
              {notes.length}
            </span>
          </div>
