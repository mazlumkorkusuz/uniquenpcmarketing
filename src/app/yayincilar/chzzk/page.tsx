...
const AVATAR_PALETTE = [CHZZK_COLOR, 'var(--info)', 'var(--success)', 'var(--orange)', 'var(--rose)', 'var(--danger)', 'var(--teal)', 'var(--info)']
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ ...STH, cursor: 'pointer', userSelect: 'none', color: active ? inkOf(CHZZK_COLOR) : 'var(--muted-foreground)' }}>
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          {[
            ...
            { label: 'Canlı Ort. İzleyici', value: fmt(row.live_avg_viewers), color: 'var(--primary-ink)' },
            { label: '30g Yayın Sayısı', value: row.stream_count_30d != null ? String(row.stream_count_30d) : '—', color: 'var(--orange)' },
          ].map(s => (
                ...
                {loading ? (
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: i < rows.length - 1 ? '1px solid var(--border)' : 'none', backgroundColor: isActive ? 'rgba(0,212,170,0.06)' : i % 2 === 1 ? 'var(--row)' : 'transparent', cursor: 'pointer' }}
                    >
                      ...
                      <td style={TD}><span style={{ color: 'var(--primary-ink)', fontWeight: 600, fontSize: '13px' }}>{fmt(row.live_avg_viewers)}</span></td>
                      <td style={TD}><span style={{ fontSize: '13px', color: 'var(--text-2)' }}>{row.stream_count_30d != null ? String(row.stream_count_30d) : <span style={{ color: 'var(--muted-foreground)' }}>—</span>}</span></td>
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {pageNumbers.map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  style={{ minWidth: '30px', height: '30px', borderRadius: '7px', border: `1px solid ${n === page ? CHZZK_COLOR : 'var(--border)'}`, backgroundColor: n === page ? 'rgba(0,212,170,0.15)' : 'transparent', color: n === page ? inkOf(CHZZK_COLOR) : 'var(--text-2)', fontSize: '12px', fontWeight: 600, cursor: 'pointer', padding: '0 6px' }}
                >
                  {n}
                </button>
              ))}
