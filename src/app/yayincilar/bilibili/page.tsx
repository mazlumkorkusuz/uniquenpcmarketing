...
const AVATAR_PALETTE = [BILIBILI_COLOR, '#fb7299', '#2563EB', '#16A34A', '#D97706', '#DB2777', '#6D28D9', '#EA580C']
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ ...STH, cursor: 'pointer', userSelect: 'none', color: active ? BILIBILI_COLOR : '#6B6B6B' }}>
  ...
  return (
    <div style={{ width: '380px', backgroundColor: '#FFFFFF', borderLeft: '1px solid #E0E0E0', display: 'flex', flexDirection: 'column', overflowY: 'auto', position: 'fixed', top: 0, right: 0, height: '100vh', zIndex: 1000 }}>
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        {/* Stats */}
        <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E0E0E0', borderRadius: '8px', padding: '12px 14px' }}>
          <div style={{ fontSize: '10px', color: '#6B6B6B', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '5px' }}>Takipçi</div>
          <div style={{ fontSize: '18px', fontWeight: 800, color: '#16A34A' }}>{fmt(row.followers)}</div>
        </div>
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          ...
          <div style={statCardStyle('#16A34A')}>
            <StatCardHeader icon={<TrendingUp size={17} />} color="#16A34A" label="Ortalama Takipçi" />
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden' }}>
          ...
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={4} style={{ padding: '48px', textAlign: 'center', color: '#6B6B6B', fontSize: '14px' }}>Yükleniyor...</td></tr>
                ) : rows.length === 0 ? (
                  <tr><td colSpan={4} style={{ padding: '48px', textAlign: 'center', color: '#6B6B6B', fontSize: '14px' }}>{hasFilters ? 'Eşleşen hesap bulunamadı' : 'Henüz hesap eklenmemiş'}</td></tr>
                ) : rows.map((row, i) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: i < rows.length - 1 ? '1px solid #E5E5E5' : 'none', backgroundColor: isActive ? 'rgba(0,161,214,0.06)' : i % 2 === 1 ? '#F9F9F9' : 'transparent', cursor: 'pointer' }}
                    >
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {pageNumbers.map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  style={{ minWidth: '30px', height: '30px', borderRadius: '7px', border: `1px solid ${n === page ? BILIBILI_COLOR : '#E0E0E0'}`, backgroundColor: n === page ? 'rgba(0,161,214,0.15)' : 'transparent', color: n === page ? BILIBILI_COLOR : '#444444', fontSize: '12px', fontWeight: 600, cursor: 'pointer', padding: '0 6px' }}
                >
                  {n}
                </button>
              ))}
