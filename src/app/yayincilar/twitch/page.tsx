...
async function getData() {
  const { data } = await supabase.from('twitch_streamers').select('*').order('followers', { ascending: false })
  return (data ?? []) as Row[]
}
...
export default async function TwitchPage() {
  ...
  const chartData = rows.slice(0, 10).map((r) => ({ label: String(r.display_name ?? r.username ?? '—'), value: Number(r.followers) || 0 }))
  ...
  const cols = [
    ...
    { key: 'id', label: '', width: '80px', render: (_: unknown, row: Row) => (
      <div style={{ display: 'flex', gap: '4px' }}>
        <EditStreamerButton row={row} table="twitch_streamers" color="#9146ff" />
        <DeleteButton table="twitch_streamers" id={row.id as string} />
      </div>
    )},
  ]
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        {chartData.length > 0 && (
          <div style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: '12px', padding: '20px', marginBottom: '28px' }}>
            ...
            <BarChart data={chartData} color="#9146ff" height={140} maxBars={10} />
          </div>
        )}
        ...
        <div style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: '12px', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #2a2a3a', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#9146ff' }} />
            <span style={{ fontSize: '15px', fontWeight: 600, color: '#f1f5f9' }}>Tüm Yayıncılar</span>
            <span style={{ marginLeft: 'auto', backgroundColor: 'rgba(145,70,255,0.12)', color: '#a78bfa', border: '1px solid rgba(145,70,255,0.3)', borderRadius: '9999px', padding: '2px 10px', fontSize: '13px', fontWeight: 600 }}>{total}</span>
          </div>
          <DataTable columns={cols} data={rows} emptyMessage="Twitch yayıncısı bulunamadı" />
        </div>
      </div>
    </div>
  )
