...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ backgroundColor: '#13131a', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.06em', padding: '11px 14px', textAlign: 'left' as const, borderBottom: '1px solid #2a2a3a', whiteSpace: 'nowrap' as const, cursor: 'pointer', userSelect: 'none' as const, color: active ? '#9146ff' : '#64748b' }}>
  ...
  return (
    <div style={{ width: '360px', backgroundColor: '#13131a', borderLeft: '1px solid #2a2a3a', display: 'flex', flexDirection: 'column', overflowY: 'auto', position: 'fixed', top: 0, right: 0, height: '100vh', zIndex: 1000 }}>
      ...
      <div style={{ padding: '16px 18px', borderBottom: '1px solid #2a2a3a', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div style={{ width: '44px', height: '44px', borderRadius: '12px', backgroundColor: 'rgba(145,70,255,0.18)', border: '2px solid rgba(145,70,255,0.35)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: '16px', color: '#a78bfa', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {!!row.display_name && String(row.display_name) !== String(row.username) && (
...
export default function TwitchPage() {
  ...
  const deleteStreamer = useCallback(async (id: unknown) => {
    await createSupabaseBrowserClient().from('twitch_streamers').delete().eq('id', id)
    ...
    <div>
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '28px' }}>
          {[
            { label: 'Toplam Yayıncı',      value: streamers.length,                        color: '#a78bfa' },
        ...
        <div style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: '12px', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #2a2a3a', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: '#9146ff', boxShadow: '0 0 6px rgba(145,70,255,0.5)' }} />
            <span style={{ fontSize: '15px', fontWeight: 600, color: '#f1f5f9' }}>Tüm Yayıncılar</span>
            <span style={{ marginLeft: 'auto', backgroundColor: 'rgba(145,70,255,0.12)', color: '#a78bfa', border: '1px solid rgba(145,70,255,0.3)', borderRadius: '9999px', padding: '2px 10px', fontSize: '13px', fontWeight: 600 }}>
                ...
                {loading ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={TD}>
                        <div style={{ fontWeight: 700, fontSize: '13px', color: '#a78bfa' }}>{String(row.username ?? '—')}</div>
                        {!!row.display_name && String(row.display_name) !== String(row.username) && (
                          <div style={{ fontSize: '11px', color: '#64748b', marginTop: '2px' }}>@{String(row.display_name)}</div>
                        )}
                      </td>
                      <td style={TD}>
                        <span style={{ color: '#a78bfa', fontWeight: 600, fontSize: '13px' }}>{fmt(row.avg_viewers)}</span>
                      </td>
                      <td style={TD}>
