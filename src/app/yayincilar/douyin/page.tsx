...
const AVATAR_PALETTE = [DOUYIN_COLOR, '#0F766E', '#1D4ED8', '#047857', '#B45309', '#BE123C', '#6D28D9', '#C2410C']
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ ...STH, cursor: 'pointer', userSelect: 'none', color: active ? inkOf(DOUYIN_COLOR) : '#655F7D' }}>
...
function ContactRow({ href, icon, label, value }: { href?: string; icon: React.ReactNode; label: string; value: string }) {
  ...
  const style: React.CSSProperties = { display: 'flex', alignItems: 'flex-start', gap: '8px', fontSize: '14px', padding: '5px 0', lineHeight: 1.5 }
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
          {[
            ...
            { label: 'Ort. Beğeni 120g', value: fmt(row.avg_likes_120d), color: '#0F766E' },
          ].map(s => (
            <div key={s.label} style={{ backgroundColor: '#FFFFFF', border: '1px solid #E8E4F1', borderRadius: '8px', padding: '12px 12px' }}>
        ...
        {/* Profile link */}
        {!!row.profile_url && (
          <a href={String(row.profile_url)} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px', borderRadius: '8px', backgroundColor: 'rgba(254,44,85,0.08)', border: '1px solid rgba(254,44,85,0.3)', color: inkOf(DOUYIN_COLOR), fontWeight: 600, fontSize: '14px', textDecoration: 'none' }}>
        ...
        {hasValue(row.bio_en) && (
          <div>
            ...
            <div style={{ fontSize: '12.5px', color: '#4A4462', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{String(row.bio_en)}</div>
          </div>
        )}
        ...
        <div>
          ...
          {contacts.length > 0 ? (
            ...
            <div style={{ fontSize: '12.5px', color: '#655F7D' }}>İletişim bilgisi yok</div>
          )}
        </div>
      </div>
    </div>
...
export default function DouyinPage() {
  ...
  const toggleStyle = (on: boolean, rgb: string, color: string): React.CSSProperties => ({
    padding: '7px 14px', borderRadius: '8px', border: `1px solid ${on ? `rgba(${rgb},0.5)` : '#E8E4F1'}`, backgroundColor: on ? `rgba(${rgb},0.1)` : 'transparent', color: on ? inkOf(color) : '#655F7D', fontSize: '14px', fontWeight: 600, cursor: 'pointer',
  })
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', padding: '12px 20px', borderBottom: '1px solid #E8E4F1', alignItems: 'center' }}>
            ...
            {hasFilters && (
              <button onClick={() => { setSearchInput(''); setSearch(''); setEmailOnly(false); setContactOnly(false) }} style={{ fontSize: '12px', color: '#655F7D', background: 'none', border: '1px solid #E8E4F1', borderRadius: '8px', padding: '7px 12px', cursor: 'pointer' }}>
                Temizle
              </button>
            )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={6} style={{ padding: '48px', textAlign: 'center', color: '#655F7D', fontSize: '14px' }}>{hasFilters ? 'Eşleşen hesap bulunamadı' : 'Henüz hesap eklenmemiş'}</td></tr>
                ) : rows.map((row, i) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: i < rows.length - 1 ? '1px solid #E8E4F1' : 'none', backgroundColor: isActive ? 'rgba(254,44,85,0.06)' : i % 2 === 1 ? '#FAF9FD' : 'transparent', cursor: 'pointer' }}
                    >
                      ...
                      <td style={{ ...TD, maxWidth: '280px' }}>
                        <a
                          ...
                          style={{ fontWeight: 700, fontSize: '14px', color: inkOf(DOUYIN_COLOR), textDecoration: 'none', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                        >
                          {String(row.channel_name ?? '—')}
                        </a>
                      </td>
                      <td style={TD}><span style={{ color: '#047857', fontWeight: 600, fontSize: '14px' }}>{fmt(row.followers)}</span></td>
                      <td style={TD}><span style={{ color: inkOf(DOUYIN_COLOR), fontWeight: 600, fontSize: '14px' }}>{fmt(row.total_likes)}</span></td>
                      <td style={TD}><span style={{ color: '#0F766E', fontWeight: 600, fontSize: '14px' }}>{fmt(row.avg_likes_120d)}</span></td>
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {pageNumbers.map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  style={{ minWidth: '30px', height: '30px', borderRadius: '8px', border: `1px solid ${n === page ? DOUYIN_COLOR : '#E8E4F1'}`, backgroundColor: n === page ? 'rgba(254,44,85,0.15)' : 'transparent', color: n === page ? inkOf(DOUYIN_COLOR) : '#4A4462', fontSize: '12px', fontWeight: 600, cursor: 'pointer', padding: '0 6px' }}
                >
                  {n}
                </button>
              ))}
