...
const AVATAR_PALETTE = [DOUYIN_COLOR, '#25f4ee', '#60a5fa', '#4ade80', '#fbbf24', '#f472b6', '#a78bfa', '#fb923c']
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ ...STH, cursor: 'pointer', userSelect: 'none', color: active ? DOUYIN_COLOR : '#64748b' }}>
...
function ContactRow({ href, icon, label, value }: { href?: string; icon: React.ReactNode; label: string; value: string }) {
  ...
  if (!href) return <div style={{ ...style, color: '#cbd5e1' }}>{content}</div>
  return <a href={href} style={{ ...style, color: '#60a5fa', textDecoration: 'none' }}>{content}</a>
}
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
          {[
            ...
            <div key={s.label} style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: '8px', padding: '12px 12px' }}>
        ...
        {/* Profile link */}
        {!!row.profile_url && (
          <a href={String(row.profile_url)} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px', borderRadius: '8px', backgroundColor: 'rgba(254,44,85,0.08)', border: '1px solid rgba(254,44,85,0.3)', color: DOUYIN_COLOR, fontWeight: 600, fontSize: '13px', textDecoration: 'none' }}>
        ...
        <div>
          ...
          {contacts.length > 0 ? (
            ...
            <div style={{ fontSize: '13px', color: '#64748b' }}>İletişim bilgisi yok</div>
          )}
        </div>
      </div>
    </div>
...
export default function DouyinPage() {
  ...
  const toggleStyle = (on: boolean, rgb: string, color: string): React.CSSProperties => ({
    padding: '7px 14px', borderRadius: '7px', border: `1px solid ${on ? `rgba(${rgb},0.5)` : '#2a2a3a'}`, backgroundColor: on ? `rgba(${rgb},0.1)` : 'transparent', color: on ? color : '#64748b', fontSize: '13px', fontWeight: 600, cursor: 'pointer',
  })
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          ...
          <div style={statCardStyle('#a78bfa')}>
            <StatCardHeader icon={<Contact size={17} />} color="#a78bfa" label="En Az 1 İletişim Olan" />
        ...
        <div style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: '12px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', padding: '12px 20px', borderBottom: '1px solid #2a2a3a', alignItems: 'center' }}>
            ...
            <button onClick={() => setEmailOnly(v => !v)} style={toggleStyle(emailOnly, '251,191,36', '#fbbf24')}>
              📧 Email Var
            </button>
            <button onClick={() => setContactOnly(v => !v)} style={toggleStyle(contactOnly, '167,139,250', '#a78bfa')}>
              💬 İletişim Var
            </button>
            {hasFilters && (
              <button onClick={() => { setSearchInput(''); setSearch(''); setEmailOnly(false); setContactOnly(false) }} style={{ fontSize: '12px', color: '#64748b', background: 'none', border: '1px solid #2a2a3a', borderRadius: '7px', padding: '7px 12px', cursor: 'pointer' }}>
                Temizle
              </button>
            )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={6} style={{ padding: '48px', textAlign: 'center', color: '#64748b', fontSize: '14px' }}>{hasFilters ? 'Eşleşen hesap bulunamadı' : 'Henüz hesap eklenmemiş'}</td></tr>
                ) : rows.map((row, i) => {
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: i < rows.length - 1 ? '1px solid rgba(42,42,58,0.6)' : 'none', backgroundColor: isActive ? 'rgba(254,44,85,0.06)' : i % 2 === 1 ? 'rgba(255,255,255,0.012)' : 'transparent', cursor: 'pointer' }}
                    >
                      ...
                      <td style={{ ...TD, maxWidth: '280px' }}>
                        <a
                          ...
                          style={{ fontWeight: 700, fontSize: '13px', color: DOUYIN_COLOR, textDecoration: 'none', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                        >
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {pageNumbers.map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  style={{ minWidth: '30px', height: '30px', borderRadius: '7px', border: `1px solid ${n === page ? DOUYIN_COLOR : '#2a2a3a'}`, backgroundColor: n === page ? 'rgba(254,44,85,0.15)' : 'transparent', color: n === page ? DOUYIN_COLOR : '#94a3b8', fontSize: '12px', fontWeight: 600, cursor: 'pointer', padding: '0 6px' }}
                >
                  {n}
                </button>
              ))}
