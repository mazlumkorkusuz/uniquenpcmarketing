...
const AVATAR_PALETTE = ['#3b82f6', '#2563EB', '#16A34A', '#D97706', '#DB2777', '#DC2626', '#0891B2', '#4F46E5']
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ ...STH, cursor: 'pointer', userSelect: 'none', color: active ? SOOP_COLOR : '#6B6B6B' }}>
  ...
  return (
    <div style={{ width: '380px', backgroundColor: '#FFFFFF', borderLeft: '1px solid #E0E0E0', display: 'flex', flexDirection: 'column', overflowY: 'auto', position: 'fixed', top: 0, right: 0, height: '100vh', zIndex: 1000 }}>
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        {!!row.bio_en && (
          <div>
            <div style={{ fontSize: '11px', fontWeight: 600, color: '#6B6B6B', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>Bio (EN)</div>
                ...
                {loading ? (
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: i < rows.length - 1 ? '1px solid #E5E5E5' : 'none', backgroundColor: isActive ? 'rgba(59,130,246,0.06)' : i % 2 === 1 ? '#F9F9F9' : 'transparent', cursor: 'pointer' }}
                    >
                      ...
                      <td style={TD}><span style={{ fontSize: '13px', color: '#444444' }}>{row.vod_count != null ? String(row.vod_count) : <span style={{ color: '#6B6B6B' }}>—</span>}</span></td>
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {pageNumbers.map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  style={{ minWidth: '30px', height: '30px', borderRadius: '7px', border: `1px solid ${n === page ? SOOP_COLOR : '#E0E0E0'}`, backgroundColor: n === page ? 'rgba(59,130,246,0.15)' : 'transparent', color: n === page ? SOOP_COLOR : '#444444', fontSize: '12px', fontWeight: 600, cursor: 'pointer', padding: '0 6px' }}
                >
                  {n}
                </button>
              ))}
