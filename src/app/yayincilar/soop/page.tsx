...
// splits a possibly comma-separated multi-value field into individual hrefs
...
const AVATAR_PALETTE = ['#3b82f6', '#70B8FF', '#3DD68C', '#FFB224', '#FF8DCC', '#FF6369', '#4CCCE6', '#9EB1FF']
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ ...STH, cursor: 'pointer', userSelect: 'none', color: active ? SOOP_COLOR : '#8F8F8F' }}>
...
function DetailPanel({ row, onClose }: { row: Row; onClose: () => void }) {
  ...
  const socialMeta: Record<(typeof SOCIAL_FIELDS)[number], { prefix: string; label: string }> = {
    ...
    kakao: { prefix: '', label: 'Kakao' },
    wechat: { prefix: '', label: 'WeChat' },
  }
  ...
  if (hasValue(row.twitch)) {
    for (const val of splitLinks(row.twitch)) {
      contacts.push({ href: toHref(val, 'https://twitch.tv/'), icon: <Globe size={13} />, label: `Twitch: ${val}` })
    }
  }
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          {[
            ...
            { label: 'VOD Sayısı', value: row.vod_count != null ? String(row.vod_count) : '—', color: '#BAA7FF' },
          ].map(s => (
        ...
        {/* Channel link */}
        {!!row.channel_url && (
          <a href={String(row.channel_url)} target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '10px', borderRadius: '8px', backgroundColor: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.3)', color: SOOP_COLOR, fontWeight: 600, fontSize: '13px', textDecoration: 'none' }}>
        ...
        {!!row.bio_en && (
          <div>
            <div style={{ fontSize: '11px', fontWeight: 600, color: '#8F8F8F', marginBottom: '8px' }}>Bio (EN)</div>
...
// ── main page ─────────────────────────────────────────────────────────────
export default function SOOPPage() {
    ...
    async function loadAux() {
      ...
      while (true) {
        const { data, error } = await supabase
          .from('soop_streamers')
          .select('followers, email, instagram, twitter, tiktok, youtube, discord, kakao, wechat')
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const supabase = createSupabaseBrowserClient()
      let query = supabase.from('soop_streamers').select('*', { count: 'exact' })
  ...
  const stats = useMemo(() => {
    ...
    return { totalStreamers: auxRows.length, totalFollowers, emailCount, anySocialCount }
  }, [auxRows])
  ...
  return (
    <div>
      <PageHeader title="SOOP Yayıncıları" subtitle="Yayıncı takip ve analizi" imageSrc="/icons/soop.jpeg" gradient="linear-gradient(135deg, #3b82f6, #70B8FF)" />

      <div style={{ padding: '24px 32px' }}>
        {/* Stats bar */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(200px, 1fr))', gap: '16px', marginBottom: '24px' }}>
          <div style={statCardStyle(SOOP_COLOR)}>
            <StatCardHeader icon={<Users size={17} />} color={SOOP_COLOR} label="Toplam Yayıncı" />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  <SortableTH label="VOD Ort. İzlenme" sk="vod_avg_views" active={sortKey === 'vod_avg_views'} dir={sortDir} onSort={handleSort} />
                  <SortableTH label="VOD Sayısı" sk="vod_count" active={sortKey === 'vod_count'} dir={sortDir} onSort={handleSort} />
                ...
                {loading ? (
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: i < rows.length - 1 ? '1px solid #1F1F1F' : 'none', backgroundColor: isActive ? 'rgba(59,130,246,0.06)' : i % 2 === 1 ? '#0E0E0E' : 'transparent', cursor: 'pointer' }}
                    >
                      ...
                      <td style={TD}>
                        <a
                          ...
                          style={{ fontWeight: 700, fontSize: '13px', color: SOOP_COLOR, textDecoration: 'none' }}
                        >
                      ...
                      <td style={TD}><span style={{ color: SOOP_COLOR, fontWeight: 600, fontSize: '13px' }}>{fmt(row.vod_avg_views)}</span></td>
                      <td style={TD}><span style={{ fontSize: '13px', color: '#B4B4B4' }}>{row.vod_count != null ? String(row.vod_count) : <span style={{ color: '#8F8F8F' }}>—</span>}</span></td>
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {pageNumbers.map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  style={{ minWidth: '30px', height: '30px', borderRadius: '7px', border: `1px solid ${n === page ? SOOP_COLOR : '#262626'}`, backgroundColor: n === page ? 'rgba(59,130,246,0.15)' : 'transparent', color: n === page ? SOOP_COLOR : '#B4B4B4', fontSize: '12px', fontWeight: 600, cursor: 'pointer', padding: '0 6px' }}
                >
                  {n}
                </button>
              ))}
