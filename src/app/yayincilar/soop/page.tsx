...
const AVATAR_PALETTE = ['#3b82f6', '#60a5fa', '#4ade80', '#fbbf24', '#f472b6', '#f87171', '#22d3ee', '#818cf8']
...
function SortableTH({ label, sk, active, dir, onSort }: { label: string; sk: SortKey; active: boolean; dir: SortDir; onSort: (k: SortKey) => void }) {
  return (
    <th onClick={() => onSort(sk)} style={{ ...STH, cursor: 'pointer', userSelect: 'none', color: active ? SOOP_COLOR : '#64748b' }}>
...
function ContactRow({ href, icon, label }: { href?: string; icon: React.ReactNode; label: string }) {
  ...
  if (!href) {
    return <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px', color: '#94a3b8', padding: '5px 0', overflow: 'hidden' }}>{content}</div>
  }
  return (
      ...
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
          {[
            ...
            { label: 'VOD Sayısı', value: row.vod_count != null ? String(row.vod_count) : '—', color: '#a78bfa' },
          ].map(s => (
        ...
        {!!row.bio_en && (
          <div>
            <div style={{ fontSize: '11px', fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>Bio (EN)</div>
            <div style={{ fontSize: '13px', color: '#94a3b8', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{String(row.bio_en)}</div>
          </div>
        )}
...
export default function SOOPPage() {
  ...
  return (
    <div>
      <PageHeader title="SOOP Yayıncıları" subtitle="Yayıncı takip ve analizi" imageSrc="/icons/soop.jpeg" gradient="linear-gradient(135deg, #3b82f6, #1d4ed8)" />
                ...
                {loading ? (
                  ...
                  return (
                    <tr
                      ...
                      style={{ borderBottom: i < rows.length - 1 ? '1px solid rgba(42,42,58,0.6)' : 'none', backgroundColor: isActive ? 'rgba(59,130,246,0.06)' : i % 2 === 1 ? 'rgba(255,255,255,0.012)' : 'transparent', cursor: 'pointer' }}
                    >
                      ...
                      <td style={TD}><span style={{ fontSize: '13px', color: '#cbd5e1' }}>{row.vod_count != null ? String(row.vod_count) : <span style={{ color: '#64748b' }}>—</span>}</span></td>
            ...
            <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
              ...
              {pageNumbers.map(n => (
                <button
                  key={n}
                  onClick={() => setPage(n)}
                  style={{ minWidth: '30px', height: '30px', borderRadius: '7px', border: `1px solid ${n === page ? SOOP_COLOR : '#2a2a3a'}`, backgroundColor: n === page ? 'rgba(59,130,246,0.15)' : 'transparent', color: n === page ? SOOP_COLOR : '#94a3b8', fontSize: '12px', fontWeight: 600, cursor: 'pointer', padding: '0 6px' }}
                >
                  {n}
                </button>
              ))}
