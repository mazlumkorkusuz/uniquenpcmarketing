...
export default async function ToplantilarPage() {
  ...
  const meetingCols = [
    { key: 'title', label: 'Toplantı Başlığı', width: '220px', render: (v: unknown) => <span style={{ fontWeight: 600, color: 'var(--foreground)' }}>{String(v ?? '—')}</span> },
    { key: 'date', label: 'Tarih', render: formatDate },
    { key: 'time', label: 'Saat', render: (v: unknown) => v ? <span style={{ fontFamily: 'monospace', color: 'var(--text-2)' }}>{String(v)}</span> : <span style={{ color: 'var(--muted-foreground)' }}>—</span> },
    ...
    { key: 'attendees', label: 'Katılımcılar', render: (v: unknown) => v ? <span style={{ fontSize: '12px', color: 'var(--text-2)', maxWidth: '200px', display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{String(v)}</span> : <span style={{ color: 'var(--muted-foreground)' }}>—</span> },
  ...
  const notesCols = [
    { key: 'meeting_id', label: 'Toplantı', render: (v: unknown) => {
      ...
      return <span style={{ fontWeight: 600, color: 'var(--foreground)' }}>{meeting ? String(meeting.title ?? '—') : <span style={{ color: 'var(--muted-foreground)' }}>—</span>}</span>
    }},
    { key: 'content', label: 'Not İçeriği', render: (v: unknown) => (
      <span style={{ fontSize: '13px', color: 'var(--text-2)', display: 'block', whiteSpace: 'pre-wrap' }}>
        {String(v ?? '—')}
      </span>
    )},
    { key: 'author', label: 'Yazan', render: (v: unknown) => v ? <Badge variant="gray">{String(v)}</Badge> : <span style={{ color: 'var(--muted-foreground)' }}>—</span> },
    {
      key: 'created_at',
      label: 'Tarih',
      render: (v: unknown) =>
  ...
  const summaryCards = [
    { label: 'Toplam', value: meetings.length, color: 'var(--primary-ink)' },
    { label: 'Yaklaşan', value: upcoming.length, color: 'var(--info)' },
    { label: 'Tamamlanan', value: past.length, color: 'var(--success)' },
    { label: 'Notlar', value: notes.length, color: 'var(--orange)' },
  ]
      ...
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '28px' }}>
          {summaryCards.map((c) => (
            ...
            >
              <div style={{ fontSize: '32px', fontWeight: 800, color: inkOf(c.color), lineHeight: 1 }}>{c.value}</div>
              <div style={{ fontSize: '13px', color: 'var(--muted-foreground)', marginTop: '6px' }}>{c.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '8px', height: '8px', borderRadius: '50%', backgroundColor: 'var(--success)', boxShadow: '0 0 6px rgba(34,197,94,0.6)' }} />
            <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--foreground)' }}>Yaklaşan Toplantılar</span>
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--foreground)' }}>Geçmiş Toplantılar</span>
            <span style={{ marginLeft: 'auto', backgroundColor: 'color-mix(in srgb, var(--muted-foreground) 12%, transparent)', color: 'var(--text-2)', border: '1px solid color-mix(in srgb, var(--muted-foreground) 30%, transparent)', borderRadius: '9999px', padding: '2px 10px', fontSize: '13px', fontWeight: 600 }}>
              {past.length}
            </span>
          </div>
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ fontSize: '15px', fontWeight: 600, color: 'var(--foreground)' }}>Toplantı Notları</span>
            <span style={{ marginLeft: 'auto', backgroundColor: 'color-mix(in srgb, var(--warning) 12%, transparent)', color: 'var(--orange)', border: '1px solid color-mix(in srgb, var(--warning) 30%, transparent)', borderRadius: '9999px', padding: '2px 10px', fontSize: '13px', fontWeight: 600 }}>
              {notes.length}
            </span>
          </div>
