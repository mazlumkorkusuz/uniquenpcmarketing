...
function scoreColor(s?: number | null) {
  if (!s) return '#655F7D'
  return s >= 80 ? '#047857' : s >= 60 ? '#B45309' : '#B91C1C'
}
...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E8E4F1', borderRadius: '8px', padding: '8px 12px', fontSize: '12px', color: '#17122B' }}>
      <div style={{ color: '#4A4462', marginBottom: '4px' }}>{label}</div>
  ...
  if (selected) {
    ...
    return (
      <div style={{ overflowX: 'hidden', maxWidth: '100%' }}>
        <div style={{ padding: '16px 28px', borderBottom: '1px solid #E8E4F1', backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: '48px', textAlign: 'center', color: '#B91C1C', fontSize: '14px' }}>{error}</div>
        ) : (
          <div style={{ display: 'flex', gap: '20px', padding: '24px 28px', alignItems: 'flex-start' }}>
            {/* LEFT COLUMN */}
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ position: 'relative', width: '100%', aspectRatio: '460/215', borderRadius: '12px', overflow: 'hidden', border: '1px solid #E8E4F1' }}>
              ...
              {(devs.length > 0 || pubs.length > 0) && (
                <div style={CARD_STYLE}>
                  {devs.length > 0 && <div style={{ marginBottom: pubs.length ? '12px' : 0 }}>
                    ...
                    {devs.map(d => <div key={d} style={{ fontSize: '12px', color: '#17122B', fontWeight: 500 }}>{d}</div>)}
                  </div>}
                  {pubs.length > 0 && <div>
                    <div style={SECTION_TITLE}>Yayıncı</div>
                    {pubs.map(p => <div key={p} style={{ fontSize: '12px', color: '#17122B', fontWeight: 500 }}>{p}</div>)}
                  </div>}
                </div>
              )}
              {s?.supportedLanguages && (
                <Collapsible title="Dil Desteği" icon={Globe}>
                  <p style={{ fontSize: '12px', color: '#4A4462', lineHeight: 1.65, margin: 0 }}>
  ...
  return (
    <div style={{ overflowX: 'hidden', maxWidth: '100%' }}>
      <style>{`
        ...
        .tot-btn:hover      { border-color: #6D28D9 !important; }
        .search-input:focus { border-color: #6D28D9 !important; }
        .history-card:hover { border-color: #6D28D9 !important; }
        .result-card:hover  { border-color: #6D28D9 !important; transform: translateY(-2px); }
      `}</style>
      ...
      <div style={{ padding: '28px 32px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#655F7D', marginBottom: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', alignItems: 'center' }}>
          <div style={{ flex: 1, maxWidth: '560px', position: 'relative' }}>
            ...
            <input
              ...
              style={{ width: '100%', boxSizing: 'border-box', backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', padding: '13px 42px 13px 44px', fontSize: '15px', color: '#17122B', outline: 'none', transition: 'border-color 0.15s' }}
            />
            {query && (
              <button onClick={() => { setQuery(''); setResults([]) }}
                style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#655F7D', display: 'flex', padding: 0 }}>
                <X size={16} />
              </button>
            )}
          </div>
        ...
        {searchHistory.length > 0 && !query && !results.length && (
          <div style={{ marginBottom: '24px' }}>
            <div style={{ fontSize: '12px', fontWeight: 600, color: '#655F7D', marginBottom: '10px' }}>
              Son Aramalar
            </div>
              ...
              {results.map(item => {
                ...
                return (
                  <button key={item.id} className="result-card" onClick={() => handleSelectGame(item)}
                    ...
                    <div style={{ position: 'relative', width: '100%', height: '120px', overflow: 'hidden', borderBottom: '1px solid #E8E4F1' }}>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
