...
const TAG_COLORS: [string, string][] = [
  ['rgba(124,58,237,0.15)', '#6D28D9'], ['rgba(59,130,246,0.15)', '#2563EB'],
  ['rgba(20,184,166,0.15)', '#0D9488'], ['rgba(249,115,22,0.15)', '#EA580C'],
  ['rgba(239,68,68,0.15)', '#DC2626'],  ['rgba(34,197,94,0.15)', '#16A34A'],
  ['rgba(245,158,11,0.15)', '#D97706'],
]

const CARD_STYLE = { backgroundColor: '#FFFFFF', border: '1px solid #E0E0E0', borderRadius: '12px', padding: '20px' }
const SECTION_TITLE: React.CSSProperties = {
  fontSize: '13px', fontWeight: 600, color: '#444444',
...
function StatCard({ icon: Icon, iconColor, iconBg, label, value }: {
  ...
  return (
    <div style={{ ...CARD_STYLE, padding: '14px 16px', display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
      ...
      <div>
        <div style={{ fontSize: '10px', color: '#6B6B6B', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '2px' }}>{label}</div>
        <div style={{ fontSize: '17px', fontWeight: 700, color: '#111111', lineHeight: 1.1 }}>{value}</div>
      </div>
    </div>
  )
}
      ...
      {entries.map(([code, val]) => {
        ...
        return (
          <div key={code} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div style={{ width: '96px', fontSize: '12px', color: '#444444', flexShrink: 0 }}>{countryName(code)}</div>
            <div style={{ flex: 1, height: '7px', backgroundColor: '#E0E0E0', borderRadius: '4px', overflow: 'hidden' }}>
              <div style={{ width: `${Math.min(barW, 100)}%`, height: '100%', background: 'linear-gradient(90deg, #7c3aed, #4f46e5)', borderRadius: '4px' }} />
            </div>
            <div style={{ width: '38px', fontSize: '11px', color: '#6B6B6B', textAlign: 'right', flexShrink: 0 }}>{pct.toFixed(1)}%</div>
          </div>
        )
      })}
    </div>
...
function ChartTooltip({ active, payload, label }: { active?: boolean; payload?: { value: number }[]; label?: string }) {
  ...
  return (
    <div style={{ backgroundColor: '#FFFFFF', border: '1px solid #E0E0E0', borderRadius: '8px', padding: '8px 12px', fontSize: '12px', color: '#111111' }}>
      <div style={{ color: '#444444', marginBottom: '4px' }}>{label}</div>
  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <LineChart data={data} margin={{ top: 4, right: 8, bottom: 0, left: 0 }}>
        ...
        <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#6B6B6B' }} tickLine={false} axisLine={false} interval={step - 1} />
        <YAxis tick={{ fontSize: 10, fill: '#6B6B6B' }} tickLine={false} axisLine={false} tickFormatter={fmtTick} width={40} />
        <Tooltip content={<ChartTooltip />} />
        <Line type="monotone" dataKey="sales" stroke="#7c3aed" strokeWidth={2} dot={false} activeDot={{ r: 4, fill: '#6D28D9' }} />
      </LineChart>
    </ResponsiveContainer>
  )
}
  ...
  return (
    <div style={CARD_STYLE}>
      <button onClick={() => setOpen(o => !o)}
        ...
        <span style={{ ...SECTION_TITLE, marginBottom: 0 }}><Icon size={13} color="#444444" /> {title}</span>
...
export default function GamalyticPage() {
  ...
  if (selected) {
    ...
    return (
      <div style={{ overflowX: 'hidden', maxWidth: '100%' }}>
        <div style={{ padding: '16px 28px', borderBottom: '1px solid #E0E0E0', backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button onClick={handleBack} style={{ display: 'flex', alignItems: 'center', gap: '5px', background: 'none', border: '1px solid #E0E0E0', borderRadius: '8px', color: '#444444', fontSize: '12px', padding: '5px 11px', cursor: 'pointer' }}>
        ...
        {loading ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '400px', flexDirection: 'column', gap: '12px' }}>
            <div style={{ width: '34px', height: '34px', border: '3px solid #E0E0E0', borderTopColor: '#7c3aed', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
            <div style={{ color: '#6B6B6B', fontSize: '13px' }}>Yükleniyor…</div>
          ...
          <div style={{ display: 'flex', gap: '20px', padding: '24px 28px', alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div style={{ position: 'relative', width: '100%', aspectRatio: '460/215', borderRadius: '12px', overflow: 'hidden', border: '1px solid #E0E0E0' }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={headerImg} alt={selected.name} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
              </div>
              <div>
                <h1 style={{ fontSize: '22px', fontWeight: 800, color: '#111111', margin: '0 0 8px' }}>{d?.name ?? selected.name}</h1>
                {(s?.shortDescription || d?.description) && (
                  <p style={{ fontSize: '13px', color: '#444444', lineHeight: 1.65, margin: 0 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
                {s?.isFree ? (
                  <span style={{ backgroundColor: 'rgba(34,197,94,0.15)', color: '#16A34A', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '8px', padding: '6px 14px', fontSize: '14px', fontWeight: 700 }}>Ücretsiz</span>
                ) : s?.priceOverview ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <span style={{ fontSize: '20px', fontWeight: 800, color: '#16A34A' }}>{s.priceOverview.finalFormatted}</span>
                    {s.priceOverview.discountPercent > 0 && <span style={{ fontSize: '13px', color: '#6B6B6B', textDecoration: 'line-through' }}>{s.priceOverview.initialFormatted}</span>}
                  </div>
                ) : d?.releaseDate ? <span style={{ fontSize: '13px', color: '#6B6B6B' }}>{d.releaseDate}</span> : null}
                {s?.metacritic && (
                  <a href={s.metacritic.url} target="_blank" rel="noopener noreferrer"
                    ...
                    <div>
                      <div style={{ fontSize: '10px', color: '#D97706', fontWeight: 600, letterSpacing: '0.04em' }}>METACRİTİC</div>
                      <div style={{ fontSize: '11px', color: '#444444' }}>İncele</div>
                    </div>
                    <ExternalLink size={12} color="#6B6B6B" />
                  </a>
                )}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(148px, 1fr))', gap: '10px' }}>
                <StatCard icon={DollarSign} iconColor="#16A34A"  iconBg="rgba(34,197,94,0.12)"   label="Tahmini Gelir"  value={fmtUSD(d?.revenue)} />
                <StatCard icon={Globe}      iconColor="#2563EB"  iconBg="rgba(59,130,246,0.12)"  label="Satılan Kopya"  value={fmt(d?.copiesSold)} />
                <StatCard icon={Star}       iconColor="#D97706"  iconBg="rgba(245,158,11,0.12)"  label="İnceleme Skoru" value={d?.reviewScore != null ? `${d.reviewScore}/100` : '—'} />
                <StatCard icon={Heart}      iconColor="#DC2626"  iconBg="rgba(239,68,68,0.12)"   label="Takipçi"        value={fmt(d?.followers)} />
                <StatCard icon={Users}      iconColor="#EA580C"  iconBg="rgba(249,115,22,0.12)"  label="Toplam Oyuncu"  value={fmt(d?.players)} />
                <StatCard icon={TrendingUp} iconColor="#6D28D9"  iconBg="rgba(124,58,237,0.12)"  label="İstek Listesi"  value={fmt(d?.wishlists)} />
                <StatCard icon={Clock}      iconColor="#0D9488"  iconBg="rgba(20,184,166,0.12)"  label="Ort. Oynama"    value={d?.avgPlaytime != null ? `${Number(d.avgPlaytime).toFixed(1)} saat` : '—'} />
              </div>
              {d?.history && d.history.length > 1 && (
                <div style={CARD_STYLE}>
                  <div style={SECTION_TITLE}><TrendingUp size={13} color="#444444" /> Satış Geçmişi</div>
                  <SalesHistoryChart history={d.history} />
                </div>
              )}
              {d?.countryData && Object.keys(d.countryData).length > 0 && (
                <div style={CARD_STYLE}>
                  <div style={SECTION_TITLE}><Globe size={13} color="#444444" /> Ülke Dağılımı (Top 10)</div>
                ...
                <Collapsible title="Sistem Gereksinimleri" icon={Cpu}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                    {s.pcRequirements.minimum && <div>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: '#DC2626', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Minimum</div>
                      <pre style={{ fontSize: '12px', color: '#444444', lineHeight: 1.7, margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'inherit' }}>{s.pcRequirements.minimum}</pre>
                    </div>}
                    {s.pcRequirements.recommended && <div>
                      <div style={{ fontSize: '11px', fontWeight: 700, color: '#16A34A', marginBottom: '6px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Önerilen</div>
                      <pre style={{ fontSize: '12px', color: '#444444', lineHeight: 1.7, margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'inherit' }}>{s.pcRequirements.recommended}</pre>
                    </div>}
                  </div>
                </Collapsible>
              )}
              {s?.dlc && s.dlc.length > 0 && (
                <div style={CARD_STYLE}>
                  <div style={SECTION_TITLE}><Gamepad2 size={13} color="#444444" /> DLC ({s.dlc.length})</div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {s.dlc.slice(0, 20).map(id => (
                      <a key={id} href={`https://store.steampowered.com/app/${id}`} target="_blank" rel="noopener noreferrer"
                        style={{ fontSize: '11px', color: '#2563EB', backgroundColor: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '5px', padding: '3px 8px', textDecoration: 'none' }}>
                        #{id}
                      </a>
                    ))}
                    {s.dlc.length > 20 && <span style={{ fontSize: '11px', color: '#6B6B6B' }}>+{s.dlc.length - 20} daha</span>}
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '14px', position: 'sticky', top: '16px' }}>
              {d?.tags && d.tags.length > 0 && (
                <div style={CARD_STYLE}>
                  <div style={SECTION_TITLE}><Tag size={13} color="#444444" /> Etiketler</div>
              ...
              {(d?.prediction1Month != null || d?.prediction1Year != null) && (
                <div style={CARD_STYLE}>
                  <div style={SECTION_TITLE}><TrendingUp size={13} color="#444444" /> Satış Tahminleri</div>
              ...
              {(genres.length > 0 || features.length > 0) && (
                <div style={CARD_STYLE}>
                  {genres.length > 0 && <>
                    <div style={SECTION_TITLE}><BarChart2 size={13} color="#444444" /> Türler</div>
                  ...
                  {features.length > 0 && <>
                    <div style={{ ...SECTION_TITLE, marginTop: genres.length ? '4px' : 0 }}><Star size={13} color="#444444" /> Özellikler</div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
                      {features.slice(0, 12).map(f => (
                        <div key={f} style={{ fontSize: '12px', color: '#444444', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              {(devs.length > 0 || pubs.length > 0) && (
                <div style={CARD_STYLE}>
                  {devs.length > 0 && <div style={{ marginBottom: pubs.length ? '12px' : 0 }}>
                    ...
                    {devs.map(d => <div key={d} style={{ fontSize: '12px', color: '#111111', fontWeight: 500 }}>{d}</div>)}
                  </div>}
                  {pubs.length > 0 && <div>
                    <div style={SECTION_TITLE}>Yayıncı</div>
                    {pubs.map(p => <div key={p} style={{ fontSize: '12px', color: '#111111', fontWeight: 500 }}>{p}</div>)}
                  </div>}
                </div>
              )}
              {s?.supportedLanguages && (
                <Collapsible title="Dil Desteği" icon={Globe}>
                  <p style={{ fontSize: '12px', color: '#444444', lineHeight: 1.65, margin: 0 }}>
  ...
  return (
    <div style={{ overflowX: 'hidden', maxWidth: '100%' }}>
      ...
      {/* Page header */}
      <div style={{ padding: '28px 32px 24px', borderBottom: '1px solid #E0E0E0', backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center', gap: '14px' }}>
        <div style={{ width: '44px', height: '44px', borderRadius: '12px', overflow: 'hidden', backgroundColor: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(0,0,0,0.12)', flexShrink: 0, padding: '6px' }}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="https://gamalytic.com/gamalytic_logo.svg" alt="Gamalytic" style={{ width: '100%', height: '100%', objectFit: 'contain' }} />
        </div>
        <div>
          <h1 style={{ fontSize: '22px', fontWeight: 800, color: '#111111', margin: 0 }}>Gamalytic</h1>
          <p style={{ fontSize: '13px', color: '#6B6B6B', margin: '2px 0 0' }}>Steam oyunu arayın, detaylı satış ve analitik verilerini görün</p>
        </div>
      </div>
      ...
      <div style={{ padding: '28px 32px' }}>
        ...
        <div style={{ marginBottom: '28px' }}>
          <div style={{ fontSize: '12px', fontWeight: 600, color: '#6B6B6B', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '10px' }}>
            🎮 Demo Vitrini
          </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', alignItems: 'center' }}>
          <div style={{ flex: 1, maxWidth: '560px', position: 'relative' }}>
            ...
            {query && (
              <button onClick={() => { setQuery(''); setResults([]) }}
                style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#6B6B6B', display: 'flex', padding: 0 }}>
                <X size={16} />
              </button>
            )}
          </div>
          ...
          >
            ...
            <span style={{ fontSize: '13px', fontWeight: 600, color: '#444444', whiteSpace: 'nowrap' }}>Tales of the Trade</span>
          </button>
        </div>
              ...
              {searchHistory.map(item => (
                <div key={item.appid} style={{ flexShrink: 0, position: 'relative' }}>
                  ...
                  >
                    ...
                    <div style={{ fontSize: '11px', fontWeight: 500, color: '#444444', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingLeft: '2px' }}>
                      {item.name}
                    </div>
                  </button>
                  <button
                    onClick={e => { e.stopPropagation(); removeFromHistory(item.appid) }}
                    style={{ position: 'absolute', top: '2px', right: '2px', width: '18px', height: '18px', borderRadius: '50%', backgroundColor: '#E0E0E0', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6B6B6B', padding: 0 }}
                  >
                    <X size={10} />
                  </button>
                </div>
        ...
        {/* ── Results / empty state ── */}
        {searching ? (
          <div style={{ textAlign: 'center', color: '#6B6B6B', fontSize: '14px', padding: '40px 0' }}>Aranıyor…</div>
        ) : results.length > 0 ? (
          <div>
            <div style={{ fontSize: '13px', color: '#6B6B6B', marginBottom: '14px' }}>{results.length} sonuç bulundu</div>
                ...
                return (
                  <button key={item.id} className="result-card" onClick={() => handleSelectGame(item)}
                    ...
                    <div style={{ padding: '12px 14px 14px' }}>
                      <div style={{ fontSize: '13px', fontWeight: 600, color: '#111111', marginBottom: '8px', lineHeight: 1.35 }}>{item.name}</div>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '6px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          ...
                          {fin != null && <span style={{ fontSize: '13px', fontWeight: 700, color: '#16A34A' }}>${fin}</span>}
                          {disc && orig && <span style={{ fontSize: '11px', color: '#6B6B6B', textDecoration: 'line-through' }}>${orig}</span>}
                          {!fin && <span style={{ fontSize: '12px', color: '#6B6B6B' }}>Ücretsiz</span>}
                        </div>
                        {item.metascore && <span style={{ fontSize: '11px', fontWeight: 700, color: '#D97706', backgroundColor: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: '5px', padding: '2px 6px' }}>MC {item.metascore}</span>}
                      </div>
                      {item.platforms && (
                        <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                          {item.platforms.windows && <span style={{ fontSize: '10px', color: '#2563EB', backgroundColor: 'rgba(59,130,246,0.1)', borderRadius: '4px', padding: '1px 6px' }}>Win</span>}
                          {item.platforms.mac     && <span style={{ fontSize: '10px', color: '#444444', backgroundColor: 'rgba(148,163,184,0.1)', borderRadius: '4px', padding: '1px 6px' }}>Mac</span>}
                          {item.platforms.linux   && <span style={{ fontSize: '10px', color: '#D97706', backgroundColor: 'rgba(245,158,11,0.1)', borderRadius: '4px', padding: '1px 6px' }}>Linux</span>}
                        </div>
                      )}
                    </div>
                  </button>
          ...
          <div style={{ textAlign: 'center', color: '#6B6B6B', fontSize: '14px', padding: '40px 0' }}>&ldquo;{query}&rdquo; için sonuç bulunamadı</div>
        ) : !searchHistory.length ? (
          <div style={{ textAlign: 'center', padding: '48px 20px' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '16px', background: 'linear-gradient(135deg, #7c3aed22, #4f46e522)', border: '1px solid #E0E0E0', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
              <BarChart2 size={28} color="#7c3aed" />
            </div>
            <div style={{ fontSize: '16px', fontWeight: 600, color: '#111111', marginBottom: '6px' }}>Oyun Analitikleri</div>
            <div style={{ fontSize: '13px', color: '#6B6B6B' }}>Aramak istediğiniz Steam oyununun adını yazın</div>
          </div>
        ) : null}
      </div>
    </div>
