...
import {
  ...
  TrendingUp, Heart, ChevronLeft, Tag, Globe, ChevronDown,
  ChevronUp, ExternalLink, Gamepad2, Cpu,
} from 'lucide-react'
import {
...
function Collapsible({ title, icon: Icon, children }: {
  ...
  return (
    <div style={CARD_STYLE}>
      ...
      >
        <span style={{ ...SECTION_TITLE, marginBottom: 0 }}>
          <Icon size={13} color="#94a3b8" /> {title}
        </span>
...
export default function GamalyticPage() {
  ...
  const handleSelectGame = useCallback(async (item: SteamItem) => {
    setSelected(item); setResults([]); setQuery(''); setGameData(null); setError(null); setLoading(true)
    try {
          ...
          <div style={{ display: 'flex', gap: '20px', padding: '24px 28px', alignItems: 'flex-start' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
                ...
                {s?.isFree ? (
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    {s.priceOverview.discountPercent > 0 && (
                      <span style={{ backgroundColor: '#22c55e', color: '#fff', borderRadius: '6px', padding: '4px 8px', fontSize: '12px', fontWeight: 700 }}>
                        -{s.priceOverview.discountPercent}%
                      </span>
                    )}
                    <span style={{ fontSize: '20px', fontWeight: 800, color: '#4ade80' }}>{s.priceOverview.finalFormatted}</span>
                    {s.priceOverview.discountPercent > 0 && (
                      <span style={{ fontSize: '13px', color: '#64748b', textDecoration: 'line-through' }}>{s.priceOverview.initialFormatted}</span>
                    )}
                  </div>
                ) : d?.releaseDate ? (
                  <span style={{ fontSize: '13px', color: '#64748b' }}>{d.releaseDate}</span>
                ) : null}
            ...
            <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '14px', position: 'sticky', top: '16px' }}>
                ...
                <div style={CARD_STYLE}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', maxHeight: '300px', overflowY: 'auto' }}>
                    {d.tags.map((tag, i) => {
                      ...
                      return (
                        <span key={tag} style={{ backgroundColor: bg, color, border: `1px solid ${color}44`, borderRadius: '5px', padding: '3px 8px', fontSize: '11px', fontWeight: 500 }}>
                          {tag}
                        </span>
                      )
                    })}
                ...
                <div style={{ ...CARD_STYLE, display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '13px', color: '#94a3b8', marginTop: '2px' }}>
                      {d.reviewScore >= 80 ? 'Çok Olumlu' : d.reviewScore >= 60 ? 'Olumlu' : 'Karışık'}
                    </div>
                  ...
                  {genres.length > 0 && (
                    <>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: features.length ? '12px' : 0 }}>
                        {genres.map(g => (
                          <span key={g} style={{ fontSize: '11px', color: '#a78bfa', backgroundColor: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: '5px', padding: '3px 8px' }}>{g}</span>
                        ))}
                      </div>
                    </>
                  )}
              ...
              {(devs.length > 0 || pubs.length > 0) && (
                <div style={CARD_STYLE}>
                  {devs.length > 0 && (
                    <div style={{ marginBottom: pubs.length ? '12px' : 0 }}>
                  ...
                  <button key={item.id} onClick={() => handleSelectGame(item)}
                    ...
                    <div style={{ padding: '12px 14px 14px' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '6px' }}>
                        ...
                        {item.metascore && (
                          <span style={{ fontSize: '11px', fontWeight: 700, color: '#fbbf24', backgroundColor: 'rgba(245,158,11,0.12)', border: '1px solid rgba(245,158,11,0.25)', borderRadius: '5px', padding: '2px 6px' }}>MC {item.metascore}</span>
                        )}
                      </div>
                      {item.platforms && (
