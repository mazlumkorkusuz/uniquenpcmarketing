              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {campaigns.map((c) => {
                    ...
                    return (
                      <tr key={c.id}>
                        <td style={{ ...tdStyle, fontWeight: 600, color: '#111111' }}>{c.name}</td>
                        <td style={tdStyle}><CampaignStatusBadge status={c.status} /></td>
                        <td style={tdStyle}>
                          <div style={{ fontSize: '12px' }}>{c.mail_accounts?.email ?? '—'}</div>
                          <div style={{ fontSize: '12px', color: '#6B6B6B' }}>{c.mail_templates?.name ?? '—'}</div>
                        </td>
                        <td style={tdStyle}><ProgressBar value={done} total={c.total_recipients} /></td>
                        <td style={tdStyle}>{c.open_count} <span style={{ color: '#6B6B6B' }}>({percent(c.open_count, c.sent_count)})</span></td>
                        <td style={tdStyle}>{c.reply_count} <span style={{ color: '#6B6B6B' }}>({percent(c.reply_count, c.sent_count)})</span></td>
                        <td style={tdStyle}>{c.bounce_count}</td>
                        <td style={{ ...tdStyle, fontSize: '12px', color: '#6B6B6B' }}>{formatDateTime(c.started_at)}</td>
