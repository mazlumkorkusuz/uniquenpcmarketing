    ...
    <div>
      ...
      <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) minmax(0, 1.4fr)', gap: '24px', alignItems: 'start' }}>
          <Card title="AI Ayarları">
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                <span style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: '#94a3b8', marginBottom: '6px' }}>Model</span>
                <div style={{ display: 'flex', gap: '4px', padding: '4px', backgroundColor: '#13131a', border: '1px solid #2a2a3a', borderRadius: '8px' }}>
            ...
            {html || editingId ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                ...
                <iframe
                  ...
                  sandbox=""
                  srcDoc={previewHtml}
                  style={{ width: '100%', height: '480px', border: '1px solid #2a2a3a', borderRadius: '8px', backgroundColor: '#fff' }}
                />
                <details>
                  <summary style={{ fontSize: '12px', color: '#94a3b8', cursor: 'pointer' }}>HTML düzenle</summary>
                  <textarea
        ...
        <Card title={`Kayıtlı Şablonlar (${templates.length})`}>
          {templates.length === 0 ? (
            <p style={{ fontSize: '13px', color: '#64748b', margin: 0 }}>Henüz kayıtlı şablon yok.</p>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '12px' }}>
              {templates.map((t) => (
                <div key={t.id} style={{ border: `1px solid ${editingId === t.id ? '#7c3aed' : '#2a2a3a'}`, borderRadius: '10px', padding: '14px', backgroundColor: '#13131a' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
                    <FileText size={14} color="#a78bfa" />
                    <span style={{ fontSize: '13px', fontWeight: 600, color: '#e2e8f0', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.name}</span>
                  </div>
                  <p style={{ fontSize: '12px', color: '#94a3b8', margin: '0 0 8px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.subject}</p>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ fontSize: '11px', color: '#64748b', flex: 1 }}>{formatDateTime(t.created_at)}</span>
