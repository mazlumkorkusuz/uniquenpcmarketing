...
function Tracking() {
  ...
  const stat = (label: string, value: number, rate: string, color: string) => (
    <div style={{ backgroundColor: '#1a1a24', border: '1px solid #2a2a3a', borderRadius: '12px', padding: '16px 20px' }}>
    ...
    <div>
      ...
      <div style={{ padding: '24px 32px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '16px' }}>
          {stat('Gönderilen', totals.sent, campaignId ? campaignName[campaignId] ?? '' : 'Tüm kampanyalar', '#60a5fa')}
          {stat('Açan', totals.opens, percent(totals.opens, totals.sent) + ' açılma', '#2dd4bf')}
          {stat('Yanıtlayan', totals.replies, percent(totals.replies, totals.sent) + ' yanıt', '#4ade80')}
          {stat('Bounce', totals.bounces, percent(totals.bounces, totals.sent + totals.bounces) + ' bounce', '#f87171')}
        </div>
        ...
        <Card padded={false}>
          {loading ? (
            <p style={{ padding: '20px', margin: 0, fontSize: '13px', color: '#64748b' }}>Yükleniyor…</p>
          ) : rows.length === 0 ? (
            <p style={{ padding: '20px', margin: 0, fontSize: '13px', color: '#64748b' }}>Kayıt yok.</p>
          ) : (
            <div style={{ overflowX: 'auto' }}>
                  ...
                  {rows.map((r) => (
                    <tr key={r.id}>
                      <td style={tdStyle}>
                        ...
                        <div style={{ fontSize: '12px', color: '#64748b' }}>{r.email}{r.platform ? ` · ${r.platform}` : ''}</div>
                      </td>
                      ...
                      <td style={{ ...tdStyle, fontSize: '12px', color: '#94a3b8' }}>{formatDateTime(r.sent_at)}</td>
                      <td style={{ ...tdStyle, fontSize: '12px', color: '#94a3b8' }}>{formatDateTime(r.opened_at)}</td>
                      <td style={tdStyle}>{r.open_count > 0 ? `${r.open_count}×` : '—'}</td>
                      <td style={{ ...tdStyle, fontSize: '12px', color: '#94a3b8' }}>
                        {r.replied_at
                          ? formatDateTime(r.replied_at)
                          : r.bounced_at
                            ? <span style={{ color: '#f87171' }}>{formatDateTime(r.bounced_at)} ({r.bounce_type})</span>
                            : '—'}
                      </td>
              ...
              {rows.length === ROW_LIMIT && (
                <p style={{ fontSize: '12px', color: '#64748b', margin: 0, padding: '10px 16px' }}>İlk {ROW_LIMIT} kayıt gösteriliyor — daraltmak için kampanya seçin veya arayın.</p>
              )}
            </div>
          )}
        </Card>
