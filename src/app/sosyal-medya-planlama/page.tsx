  ...
  return (
    <div>
      <PageHeader
        ...
        icon={CalendarCheck}
        gradient="linear-gradient(135deg, #9E8CFC, #ec4899)"
      />
      <div style={{ padding: '24px 32px' }}>
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', padding: '20px', marginBottom: '28px' }}>
          ...
          <div style={{ fontSize: '12px', color: '#8F8F8F', marginBottom: '16px' }}>Platforma göre toplam planlanan gönderi sayısı</div>
          <BarChart data={platformBar} color="#9E8CFC" height={100} />
        </div>
              ...
              >
                <div style={{ padding: '16px 20px', borderBottom: '1px solid #262626' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ fontSize: '26px', fontWeight: 800, color: count > 0 ? platform.color : '#8F8F8F' }}>{count}</span>
                  </div>
                ...
                <div style={{ padding: '14px 20px', flex: 1 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '20px', fontWeight: 700, color: '#70B8FF' }}>{sched}</div>
                      <div style={{ fontSize: '11px', color: '#8F8F8F', marginTop: '2px' }}>Planlandı</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '20px', fontWeight: 700, color: '#3DD68C' }}>{count - sched}</div>
                      <div style={{ fontSize: '11px', color: '#8F8F8F', marginTop: '2px' }}>Diğer</div>
                    </div>
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ fontSize: '20px', fontWeight: 700, color: platform.color }}>{count}</div>
                      <div style={{ fontSize: '11px', color: '#8F8F8F', marginTop: '2px' }}>Toplam</div>
                    </div>
                  </div>
                </div>
