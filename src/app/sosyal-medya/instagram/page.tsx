...
export default async function InstagramPage() {
  ...
  const postCols = [
    ...
    { key: 'campaign',    label: 'Kampanya',  render: (v: unknown) => v ? <Badge variant="orange">{String(v)}</Badge> : <span style={{ color: '#6B6B6B' }}>—</span> },
    ...
    <div>
      ...
      <div style={{ padding: '24px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '28px' }}>
          ...
          <StatCard label="Metrik Kaydı"    value={metrics.length}                          icon={TrendingUp}   iconColor="#16A34A" iconBg="rgba(34,197,94,0.12)" />
        </div>
        ...
        {chartData.length > 0 && (
          <div style={{ backgroundColor: '#FFFFFF', boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 1px 2px rgba(0,0,0,0.04)', border: '1px solid #E0E0E0', borderRadius: '12px', padding: '20px', marginBottom: '28px' }}>
            <div style={{ fontSize: '14px', fontWeight: 600, color: '#111111', marginBottom: '16px' }}>Kampanyaya Göre Gönderi Dağılımı</div>
        ...
        <div style={{ backgroundColor: '#FFFFFF', boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 1px 2px rgba(0,0,0,0.04)', border: '1px solid #E0E0E0', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #E0E0E0', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ marginLeft: 'auto', backgroundColor: 'rgba(225,48,108,0.12)', color: '#DB2777', border: '1px solid rgba(225,48,108,0.3)', borderRadius: '9999px', padding: '2px 10px', fontSize: '13px', fontWeight: 600 }}>{posts.length}</span>
          </div>
