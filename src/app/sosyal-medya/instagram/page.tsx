    ...
    <div>
      ...
      <div style={{ padding: '24px 32px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '16px', marginBottom: '28px' }}>
          ...
          <StatCard label="Toplam Yorum"    value={totalComments.toLocaleString('en-US')}   icon={MessageCircle} iconColor="#6D28D9" iconBg="#F3EEFF" />
          <StatCard label="Metrik Kaydı"    value={metrics.length}                          icon={TrendingUp}   iconColor="#047857" iconBg="#ECFDF5" />
        </div>
        ...
        <div style={{ backgroundColor: 'var(--color-bg-card)', boxShadow: 'var(--shadow-card)', border: '1px solid var(--color-border-card)', borderRadius: '12px', overflow: 'hidden', marginBottom: '24px' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #E8E4F1', display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <span style={{ marginLeft: 'auto', backgroundColor: 'rgba(225,48,108,0.12)', color: '#BE123C', border: '1px solid rgba(225,48,108,0.3)', borderRadius: '9999px', padding: '2px 10px', fontSize: '14px', fontWeight: 600 }}>{posts.length}</span>
          </div>
