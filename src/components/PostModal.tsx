...
export function PostModal({ platform, platformColor, mode = 'add', initialData, open: externalOpen, onClose: externalClose }: PostModalProps) {
  ...
  const modal: React.CSSProperties = {
    backgroundColor: '#0A0A0A', border: '1px solid #262626', borderRadius: '14px',
    width: '100%', maxWidth: '480px', padding: '28px', position: 'relative',
    boxShadow: '0 25px 60px rgba(0,0,0,0.12)',
  }
  const label: React.CSSProperties = { display: 'block', fontSize: '12px', fontWeight: 600, color: '#B4B4B4', marginBottom: '6px', }
  const input: React.CSSProperties = { width: '100%', backgroundColor: '#0A0A0A', border: '1px solid #262626', borderRadius: '8px', padding: '10px 12px', fontSize: '14px', color: '#EDEDED', outline: 'none', boxSizing: 'border-box' }
    ...
    <>
      ...
      {open && (
        <div style={overlay} onClick={(e) => e.target === e.currentTarget && close()}>
          <div style={modal}>
            <button onClick={close} style={{ position: 'absolute', top: '18px', right: '18px', background: 'none', border: 'none', color: '#8F8F8F', cursor: 'pointer', padding: '2px', display: 'flex', alignItems: 'center' }}>
              <X size={18} />
            </button>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              <h2 style={{ fontSize: '18px', fontWeight: 700, color: '#EDEDED', margin: 0, marginBottom: '4px' }}>
                {mode === 'edit' ? 'Gönderiyi Düzenle' : 'Gönderi Ekle'}
              </h2>
              <p style={{ fontSize: '13px', color: '#8F8F8F', margin: 0 }}>{platform.charAt(0).toUpperCase() + platform.slice(1)} için içerik {mode === 'edit' ? 'düzenle' : 'planla'}</p>
            </div>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                ...
                >
                  {STATUS_OPTIONS.map(s => <option key={s} value={s} style={{ backgroundColor: '#0A0A0A' }}>{s}</option>)}
                </select>
              </div>

              {error && (
                <div style={{ padding: '10px 12px', backgroundColor: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: '8px', fontSize: '13px', color: '#FF6369' }}>
                  {error}
                </div>
              )}

              <div style={{ display: 'flex', gap: '10px', paddingTop: '4px' }}>
                <button type="button" onClick={close} style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #262626', backgroundColor: 'transparent', color: '#8F8F8F', fontSize: '14px', fontWeight: 500, cursor: 'pointer' }}>
                  İptal
                </button>
                <button
                  type="submit"
